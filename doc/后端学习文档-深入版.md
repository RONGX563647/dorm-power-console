...
| **认证** | `/api/devices/**`, `/api/telemetry/**` 等 | 需要有效的JWT令牌 |
...
**设计亮点**:
- ✅ **线程安全**: 使用`ConcurrentHashMap`、`AtomicInteger`、`volatile`保证多线程安全
- ✅ **资源控制**: 固定线程池 + 设备数量限制，防止资源耗尽
- ✅ **优雅停止**: 使用`volatile boolean`标志位实现可控停止
...
- [Java 21语言特性](https://openjdk.org/projects/jdk/21/)
