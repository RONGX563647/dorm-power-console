...
public class IntentRecognizer {
    ...
    private String getApiEndpoint(String intent, Map<String, String> entities) {
        ...
        switch (intent) {
            ...
            case "POWER_QUERY":
                return roomId.isEmpty() ? null : "/api/telemetry/room/" + roomId + "/stats";
            case "DEVICE_STATUS":
                return roomId.isEmpty() ? null : "/api/devices/room/" + roomId;
            case "BILL_QUERY":
                return roomId.isEmpty() ? null : "/api/bills/room/" + roomId;
            case "ALARM_QUERY":
                return "/api/alarms";
            case "SCENE_CONTROL":
                return roomId.isEmpty() ? null : "/api/saving/strategies/" + roomId;
            case "ENERGY_SAVING":
                return roomId.isEmpty() ? null : "/api/saving/strategies/" + roomId;
            case "ROOM_MANAGEMENT":
                return roomId.isEmpty() ? "/api/dorm/rooms" : "/api/dorm/rooms/" + roomId;
            case "STUDENT_MANAGEMENT":
                return roomId.isEmpty() ? "/api/students" : "/api/students/room/" + roomId;
            case "POWER_CONTROL":
                return roomId.isEmpty() ? "/api/power-control/cutoff-rooms" : "/api/power-control/status/" + roomId;
            case "FIRMWARE_UPGRADE":
                return deviceId.isEmpty() ? "/api/firmware/pending" : "/api/firmware/device/" + deviceId;
            default:
                return null;
        }
    }
