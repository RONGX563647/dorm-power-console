...
| `deploy_scripts.sh` | 一键部署运维脚本 | 手动执行 |

## 快速部署

```bash
# 1. 复制脚本到服务器
scp -r scripts/ops/ root@your-server:/opt/dorm-power/

# 2. 设置执行权限
chmod +x /opt/dorm-power/scripts/ops/*.sh
...
```bash
# 数据库备份
/opt/dorm-power/scripts/ops/backup_db.sh

# 日志清理
/opt/dorm-power/scripts/ops/cleanup_logs.sh

# 系统监控
/opt/dorm-power/scripts/ops/monitor.sh
```
...
```bash
# 测试数据库备份
bash -x /opt/dorm-power/scripts/ops/backup_db.sh

# 测试日志清理
bash -x /opt/dorm-power/scripts/ops/cleanup_logs.sh

# 测试监控
bash -x /opt/dorm-power/scripts/ops/monitor.sh
```
